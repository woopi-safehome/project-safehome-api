...
class GlobalExceptionHandler {
    ...
    fun handleBusinessException(e: BusinessException, response: HttpServletResponse): ResponseEntity<ApiResponse<Nothing?>>? {
    ...
    fun handleValidationException(ex: MethodArgumentNotValidException, response: HttpServletResponse): ResponseEntity<ApiResponse<Nothing?>>? {
        if (isSseResponse(response)) return null
    ...
    fun handleConstraintViolation(ex: ConstraintViolationException, response: HttpServletResponse): ResponseEntity<ApiResponse<Nothing?>>? {
        if (isSseResponse(response)) return null
    ...
    fun handleGenericException(ex: Exception, response: HttpServletResponse): ResponseEntity<ApiResponse<Nothing?>>? {
        Sentry.captureException(ex)
        if (isSseResponse(response)) return null
        return createErrorResponse(ErrorCode.INTERNAL_SERVER_ERROR, ex.localizedMessage)
    }

    private fun isSseResponse(response: HttpServletResponse): Boolean =
        response.isCommitted || response.contentType?.contains("text/event-stream") == true
