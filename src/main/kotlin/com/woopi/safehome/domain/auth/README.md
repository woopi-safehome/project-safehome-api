...
> 컴파일은 되므로 코드만 보고는 드러나지 않는다.

**이 한 가지는 제약 테스트가 막는다.** 공개 목록에 없는 엔드포인트가 인증 인자를
받지 않으면 테스트가 깨진다. 공개하려면 그 목록에 의식적으로 올려야 한다.

### 인자를 비워 둘 수 있게 선언하면 인증이 강제되지 않는다

식별자 인자는 **비워 둘 수 있게도** 선언할 수 있다. 그렇게 선언한 엔드포인트는
**인증 없이도 통과한다** — 비회원을 받으려는 경로가 이 형태다.
토큰이 아예 없으면 비회원으로 들어오고, **토큰이 왔는데 유효하지 않으면 거절한다.**
만료된 토큰을 든 사용자가 자기도 모르게 비회원으로 떨어지지 않게 하기 위해서다.

> **비워 둘 수 있다는 것은 보호가 아니다.** 실수로 이 형태를 쓰면 열린 채로 남는다.
> 그래서 제약 테스트는 이런 경로도 **별도 목록에 올라와 있을 때만** 통과시킨다.
> 목록이 둘인 이유다 — 아예 공개하는 것과, 회원·비회원을 모두 받는 것.

**다만 인자를 받는 것과 쓰는 것은 다르다.** 받아 놓고 조회에 쓰지 않으면 검사를 통과하면서도
남의 자원이 열린다. 소유자 확인은 유스케이스의 몫이다.
...
**보안 프레임워크가 없다는 결정의 대가를 이 도메인이 진다.** 프레임워크가 가로채 주지 않으므로
인증이 성립하는 조건을 이 도메인이 직접 지킨다. 그중 기계가 판정할 수 있는 부분은 테스트로 옮겼고,
나머지는 사람이 본다.
