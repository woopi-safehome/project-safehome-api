...
class DocumentConstraintTest : BehaviorSpec({
    ...
    val constraintTests = filesUnder(File(root, "src/test/kotlin/com/woopi/safehome/constraint"), ".kt")
    ...
    Given("문서의 링크") {
        When("가리키는 곳을 따라가면") {
            val broken = docs.flatMap { doc ->
                link.findAll(doc.readText()).mapNotNull { m ->
                    val target = m.groupValues[1].substringBefore('#')
                    val external = target.isEmpty() || listOf("http:", "https:", "mailto:").any { target.startsWith(it) }
                    when {
                        external || leavesRepo(target, doc) -> null
                        File(doc.parentFile, target).exists() -> null
                        else -> "${rel(doc)} → ${m.groupValues[1]}"
                    }
                }.toList()
            }
    ...
    Given("문서 경로") {
        When("문서와 테스트에 적힌 경로를 찾으면") {
            ...
            val missing = docs.flatMap { doc ->
                backticked.findAll(doc.readText()).map { it.groupValues[1] }
                    .filter { !leavesRepo(it, doc) && resolveDoc(it, doc) == null }
                    .map { "${rel(doc)} → $it" }
                    .toList()
            } + testSources.flatMap { src ->
                bare.findAll(src.readText()).map { it.value }
                    .filter { !it.startsWith("../") && resolveDoc(it) == null }
                    .map { "${rel(src)} → $it" }
                    .toList()
            }
    ...
    Given("계약 절") {
        When("제목에 계약이 들어간 절을 찾으면") {
            val unmarked = docs.flatMap { doc ->
                val lines = doc.readLines()
                val headings = headingsOf(lines)
                headings.filter { "계약" in it.third }
                    .filterNot { h -> lines.subList(h.first, sectionEnd(lines, headings, h)).any { "이 절은 계약이다" in it } }
                    .map { "${rel(doc)} → ${it.third}" }
            }
    ...
    Given("문서 지도와 머리글") {
        ...
        When("스스로 문서임을 밝힌 문서를 모으면") {
            ...
            else docs.filter { d -> d != readme && d.readLines().take(15).any { it.startsWith("> **범위**") } && d !in mapped }
                .map { rel(it) }
    ...
    Given("제약 테스트") {
        When("검사마다 실패 메시지를 보면") {
            val site = Regex("""\.check\(|\bshould(?:Be|NotBe|BeEmpty|Contain\w*)\b""")
