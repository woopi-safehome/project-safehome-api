...
계층 규칙은 제약 테스트가 강제하므로, 구조를 어기면 빌드가 깨진다.
...
**그러나 배포 성공이 기동 성공은 아니다.** 컨테이너에 헬스 체크가 정의돼 있지만
배포는 그것을 기다리지 않는다. 컨테이너를 띄우라고 지시한 순간 워크플로는 끝난다.
**기동에 실패해도 배포 로그는 초록이다** — 확인하려면 서버를 봐야 한다.
