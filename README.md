...
**비회원이 만든 작업에도 주인이 있다 — 업로드한 브라우저다.** 서버가 `safehome_anon` 쿠키를
발급하고, **그 쿠키를 함께 보낸 요청만** 조회·구독할 수 있다. 식별자만으로는 열리지 않는다.
쿠키는 `HttpOnly` 라 스크립트가 읽지 못하고, 같은 사이트 요청에만 실린다(`SameSite=Lax`) —
**브라우저에서 부를 때 쿠키를 함께 보내도록** 해야 하고, 다른 도메인에서 부르면 실리지 않는다.
쿠키를 지우거나 다른 브라우저로 옮기면 자기 분석도 다시 볼 수 없다.
주인이 있는 작업은 종전대로 본인만 볼 수 있고, 비회원은 이력 목록을 가질 수 없다.

> 익명 쿠키를 쓰기 전에 만들어진 작업에는 주인이 없어, 식별자를 아는 사람이 볼 수 있다.
